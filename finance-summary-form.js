(function(){
  ...
  function addStyle(){
    ...
    const s=document.createElement('style');s.id='finance-summary-form-style';s.textContent=`
      ...
      .fsf-title{font-size:25px;font-weight:900;color:#071b3a;margin:0 0 6px;direction:rtl}
      .fsf-sub{font-size:13px;font-weight:700;color:#52667f;margin:3px 0}
      .fsf-date{font-size:15px;font-weight:900;color:#14233d;margin-top:10px}
      .fsf-body{padding:24px}
      .fsf-section{border:1px solid #cbd8e8;border-radius:8px;margin-bottom:18px;overflow:hidden}
      .fsf-section h3{margin:0;padding:11px 14px;background:#071b3a;color:#fff;font-size:16px}
      .fsf-section.income h3{background:#176b3a}.fsf-section.expense h3{background:#9f2424}
  ...
  function render(){
    ...
    result.innerHTML=`<div class="finance-summary-form">
      <div class="fsf-head"><img src="${esc(logo)}" alt="Madrasa logo"><div class="fsf-title">${esc(urdu)}</div><div class="fsf-sub">${esc(name)}</div><div class="fsf-sub">Phone: ${esc(phone)}</div><div class="fsf-date">${esc(dateText)}</div></div>
      <div class="fsf-body">
        <div class="fsf-section income"><h3>Income — ذریعہ آمدنی</h3><table class="fsf-table"><thead><tr><th>Source</th><th>Total</th></tr></thead><tbody>${makeRows(inc)}</tbody></table></div>
        <div class="fsf-section expense"><h3>Expenses — خرچ</h3><table class="fsf-table"><thead><tr><th>Expense</th><th>Total</th></tr></thead><tbody>${makeRows(exp)}</tbody></table></div>
        <div class="fsf-totals"><div class="fsf-total"><b>Total Income</b><strong>${fmt(s.it)}</strong></div><div class="fsf-total"><b>Total Expense</b><strong>${fmt(s.et)}</strong></div><div class="fsf-total current"><b>Current Total</b><strong>${fmt(s.current)}</strong></div></div>
      </div>
      <div class="fsf-foot"><strong>${esc(name)}</strong><span class="urdu">${esc(urdu)}</span><div class="fsf-note">Finance Summary</div></div>
    </div>`;
  }
  ...
  function start(){
    ...
    document.addEventListener('click',function(e){
      ...
      const run=e.target.closest('#runReport');
      if(tab||run) delayedRender();
    },true);
    const mo=new MutationObserver(()=>{if(window.__summary) render()});
    const rr=$('reportResult');if(rr)mo.observe(rr,{childList:true,subtree:true});
