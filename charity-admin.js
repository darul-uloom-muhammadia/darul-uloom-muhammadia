(()=>{
  ...
  async function admin(){
    ...
    const intro=document.createElement('section'); intro.className='card'; intro.style.marginTop='20px';
    intro.innerHTML='<div class="head"><div><h1>تعارف نامہ — Introduction</h1><p>Change the introduction heading and text directly from the Admin Dashboard.</p></div></div><div id="introMsg" class="msg"></div><div style="display:grid;gap:14px"><div><label style="display:block;font-weight:800;margin-bottom:6px">عنوان / Heading</label><input id="introHeading" style="width:100%;box-sizing:border-box;padding:12px;border:1px solid #d6e0ec;border-radius:9px;font-family:\"Noto Nastaliq Urdu\",\"Noto Naskh Arabic\",serif;direction:rtl;text-align:right" value="تعارف نامہ"></div><div><label style="display:block;font-weight:800;margin-bottom:6px">تعارف کا متن / Introduction Text</label><textarea id="introBody" style="width:100%;box-sizing:border-box;min-height:170px;padding:12px;border:1px solid #d6e0ec;border-radius:9px;font-family:\"Noto Nastaliq Urdu\",\"Noto Naskh Arabic\",serif;direction:rtl;text-align:right;line-height:2" placeholder="اپنا تعارف یہاں لکھیں..."></textarea></div><div style="display:flex;gap:10px;flex-wrap:wrap"><button id="saveIntro" class="btn green">Save Introduction</button><button id="resetIntro" class="btn light">Reset Default</button></div></div>';
    dash.parentNode.insertBefore(intro,dash.nextSibling);

    const boxMsg=(t,c='success')=>{const e=qs('#introMsg');e.textContent=t;e.className='msg show '+c};
    const defaults={heading:'تعارف نامہ',body:'جامعہ کے تعارف اور بنیادی معلومات کے لیے یہاں اپنا متن درج کریں۔'};
    async function getHomepage(){const {data,error}=await supabaseClient.from('site_settings').select('content').eq('id','homepage').maybeSingle();if(error)throw error;return data?.content||{}}
    async function loadIntro(){try{const c=await getHomepage();const t={...defaults,...(c.taaruf||{})};qs('#introHeading').value=t.heading;qs('#introBody').value=t.body}catch(e){boxMsg(e.message||'Unable to load introduction.','error')}}
    async function saveIntro(){try{const c=await getHomepage();c.taaruf=c.taaruf||{};c.taaruf.heading=qs('#introHeading').value.trim()||defaults.heading;c.taaruf.body=qs('#introBody').value.trim()||defaults.body;const {error}=await supabaseClient.from('site_settings').upsert({id:'homepage',content:c,updated_by:user.id,updated_at:new Date().toISOString()},{onConflict:'id'});if(error)throw error;boxMsg('تعارف نامہ successfully saved.')}catch(e){boxMsg(e.message||'Save failed.','error')}}
    async function resetIntro(){qs('#introHeading').value=defaults.heading;qs('#introBody').value=defaults.body;await saveIntro()}
    qs('#saveIntro').onclick=saveIntro;qs('#resetIntro').onclick=resetIntro;await loadIntro();
