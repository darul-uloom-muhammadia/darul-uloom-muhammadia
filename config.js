  ...
  async function apply(){
    try {
      ...
      if (logo && /admin\.html$|student-portal\.html$/.test(location.pathname)) {
        const loginIcon = document.querySelector('#login > div:first-child > div:first-child');
        if (loginIcon) {
          loginIcon.innerHTML = `<img src="${logo}" alt="Darul Uloom Muhammadiya logo" style="width:72px;height:72px;object-fit:contain;border-radius:12px;display:block;margin:auto">`;
          loginIcon.style.fontSize = '0';
        }
        document.querySelectorAll('.logo-mark').forEach(el => {
          el.innerHTML = `<img src="${logo}" alt="Darul Uloom Muhammadiya logo">`;
