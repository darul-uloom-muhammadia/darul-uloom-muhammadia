...
(async function () {
  ...
  function applyPublic(c) {
    ...
    if (b.logoImage) document.querySelectorAll('.logo-mark,.admin-logo,.hero-card .crescent').forEach(el=>{el.innerHTML='<img src="'+String(b.logoImage).replace(/["<>]/g,'')+'" alt="Darul Uloom Muhammadia logo">';el.classList.add('has-image');});
    ...
    window.__siteContent=c; window.__finalCmsContent=c; try{window.__applySiteContent?.(c);}catch(e){} try{window.__applyStableCms?.(c);}catch(e){}
  }
  ...
  function loadFinalCms(){if(document.getElementById('final-cms-script'))return;const s=document.createElement('script');s.id='final-cms-script';s.src='final-cms.js?v=20260816-1';s.defer=true;document.head.appendChild(s);}
  ...
  function loadCharityFix(){const p=location.pathname;if(!p.endsWith('charity.html')&&!p.endsWith('charity-submission-receipt.html'))return;if(document.getElementById('charity-security-fix'))return;const s=document.createElement('script');s.id='charity-security-fix';s.src='charity-security-fix.js?v=20260816-3';s.defer=true;document.head.appendChild(s);}
  ...
  const start=()=>{if(isHomePage){publicRefresh();loadFinalCms();}if(isAdminPage)applyAdminBrand();syncEditorFields();loadCharityFix();};
